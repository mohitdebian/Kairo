...
interface BrowserState {
  ...
  addWorkspace: (workspace: Omit<Workspace, 'id'> & { id?: string }) => void
    ...
    (set, get) => ({
      ...
      splitPane: (
        ...
        if (newTabId) {
          ...
          if (oldNode && oldNode.id !== paneId && tabId) {
            // only remove if we explicitly passed tabId (drag and drop)
      ...
      splitCurrentPane: (direction: SplitDirection, url?: string) =>
        set((state) => {
      ...
      moveTabToPane: (tabId, targetNodeId) =>
        set((state) => {
          const layout = state.layouts[state.activeWorkspaceId]
          if (!layout) return state
      ...
      toggleDownloads: () => set((state) => ({ isDownloadsOpen: !state.isDownloadsOpen })),
      ...
      setActiveWorkspace: (id) =>
        set((state) => {
          ...
          if (layout) {
            ...
            const sortedTabs = [...workspaceTabs].sort(
      ...
      addWorkspace: (workspace) =>
        set((state) => ({
          workspaces: [...state.workspaces, { ...workspace, id: workspace.id || `w${Date.now()}` }]
        })),
      deleteWorkspace: (id) =>
        set((state) => {
          const newWorkspaces = state.workspaces.filter((w) => w.id !== id)
          const newActiveWorkspaceId =
            state.activeWorkspaceId === id ? newWorkspaces[0]?.id || '' : state.activeWorkspaceId
            ...
            if (layout) {
              ...
              if (node) {
                ...
                if (newLayout) {
                  ...
                  const remainingTabsInWorkspace = state.tabs.filter(
                    (t) => t.workspaceId === state.activeWorkspaceId && t.id !== id
                  )
                  const fallbackTabId =
                    remainingTabsInWorkspace.length > 0
                      ? remainingTabsInWorkspace[remainingTabsInWorkspace.length - 1].id
                      : undefined
                  if (fallbackTabId) {
        ...
        set((state) => {
          ...
          return {
            ...
            tabs: state.tabs.map((t) =>
              t.id === id ? { ...t, isSleeping: false, lastActiveAt: Date.now() } : t
            )
          }
        }),
      ...
      moveTabToFolder: (tabId, folderId, targetId?: string) =>
        set((state) => {
      ...
      moveTabToWorkspace: (tabId, workspaceId) =>
        set((state) => ({
          tabs: state.tabs.map((t) =>
            t.id === tabId ? { ...t, workspaceId, folderId: undefined } : t
          )
        })),
      ...
      updateWorkspaceNote: (workspaceId, note) =>
        set((state) => ({
