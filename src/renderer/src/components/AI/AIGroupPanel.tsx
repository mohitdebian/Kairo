...
import {
  Sparkles, X, Check, RotateCcw, ChevronDown, ChevronUp,
  Merge, Folder, GripVertical, ArrowRight
} from 'lucide-react'
...
const TabPreview = ({ tabId, groupId }: { tabId: string; groupId: string }) => {
  ...
  const favicon = tab.favicon ||
    (tab.url?.startsWith('http') ? `https://www.google.com/s2/favicons?domain=${new URL(tab.url).hostname}&sz=32` : null)
  ...
  return (
    ...
    >
      ...
      <div className="w-4 h-4 shrink-0 flex items-center justify-center">
        {favicon ? (
          <img src={favicon} alt="" className="w-3.5 h-3.5 object-contain rounded-sm" onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
        ) : (
        ...
        <div className="relative">
          ...
          {showMove && (
            <div className="absolute right-0 bottom-full mb-1 z-50 bg-[#18181b] border border-white/10 rounded-lg shadow-xl overflow-hidden min-w-[140px] py-1">
              {suggestions.filter(g => g.id !== groupId).map(g => (
                <button
                  key={g.id}
                  onClick={() => { moveTabBetweenSuggestions(tabId, g.id); setShowMove(false) }}
...
const GroupCard = ({ group, index, total }: { group: AIGroup; index: number; total: number }) => {
  ...
  const { renameSuggestion, renameSuggestionEmoji, mergeSuggestions, suggestions } = useAIGroupStore()
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative">
          <button
            onClick={e => { e.stopPropagation(); setIsEditingEmoji(v => !v) }}
          ...
          {isEditingEmoji && (
            ...
            >
              {EMOJI_OPTIONS.map(em => (
                <button
                  key={em}
                  onClick={() => { renameSuggestionEmoji(group.id, em); setIsEditingEmoji(false) }}
        ...
        {isEditingName ? (
          <input
            ...
            onKeyDown={e => {
              ...
              if (e.key === 'Escape') { setIsEditingName(false); setNameInput(group.name) }
            }}
            onClick={e => e.stopPropagation()}
...
export const AIGroupPanel = () => {
  const { isPanelOpen, isAnalyzing, suggestions, closePanel, runAnalysis, clearSuggestions } = useAIGroupStore()

  const store = useBrowserStore()
  const { tabs, activeWorkspaceId, folders, createFolder, moveTabToFolder } = store

  const workspaceTabs = tabs.filter(t => t.workspaceId === activeWorkspaceId && t.url && t.url !== 'dashboard')
  ...
  const handleApply = () => {
    ...
    setTimeout(() => {
      ...
      for (const group of suggestions) {
        ...
        const folder = freshFolders.find(f => f.name === folderName && f.workspaceId === activeWorkspaceId)
        if (!folder) continue
        ...
        >
            ...
            <div className="px-4 pt-5 pb-4 border-b border-white/[0.05] shrink-0">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-white/40">
                      {isAnalyzing ? 'Analyzing your tabs…' : suggestions.length > 0
                ...
                <button onClick={closePanel} className="p-1.5 rounded-lg hover:bg-white/5 text-white/40 hover:text-white/70 transition-colors">
                  <X size={16} />
                </button>
              </div>
            </div>
              ...
              {isAnalyzing ? (
                ...
                <div className="flex flex-col items-center justify-center h-48 gap-4">
                  <div className="relative w-12 h-12">
                    ...
                    <div className="absolute inset-2 rounded-full border-2 border-indigo-500/20 border-b-indigo-400 animate-spin" style={{ animationDirection: 'reverse', animationDuration: '0.8s' }} />
                  </div>
                  <div className="text-center">
                    <p className="text-[13px] font-medium text-white/70">Analyzing {workspaceTabs.length} tabs</p>
                ...
                <div className="flex flex-col items-center justify-center h-48 gap-3 text-center px-4">
                  ...
                  <p className="text-[11px] text-white/25">Click "Analyze" to group your tabs automatically</p>
                  <button
