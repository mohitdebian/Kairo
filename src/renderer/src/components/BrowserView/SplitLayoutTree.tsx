...
function calculateLayout(
...
): { leaves: Array<{ node: SplitNode; inset: string }>; splitters: Array<{ parentId: string; index: number; isHorizontal: boolean; sizes: number[]; inset: string }> } {
  if (node.type === 'tab') {
    return {
      leaves: [{ node, inset: `${pos.top}% ${100 - pos.right}% ${100 - pos.bottom}% ${pos.left}%` }],
      splitters: []
    }
  }
  ...
  if (node.type === 'split' && node.children) {
    ...
    let leaves: Array<{ node: SplitNode; inset: string }> = []
    let splitters: Array<{ parentId: string; index: number; isHorizontal: boolean; sizes: number[]; inset: string }> = []
...
const DropZone = ({ side, nodeId }: { side: 'center' | 'top' | 'right' | 'bottom' | 'left', nodeId: string }) => {
  ...
  return (
    <div className="w-full h-full relative overflow-hidden">
      ...
      {splitters.map((splitter, i) => (
        <SplitDivider 
          key={`${splitter.parentId}-${splitter.index}-${i}`}
          {...splitter}
        />
      ))}
    </div>
...
const SplitDivider: React.FC<SplitDividerProps> = ({ parentId, index, isHorizontal, sizes, inset }) => {
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    const onPointerMove = (moveEvent: PointerEvent) => {
      ...
      const totalSize = parentEl ? (isHorizontal ? parentEl.clientWidth : parentEl.clientHeight) : 1000
      ...
      const diff = (newSizes[index] + newSizes[index + 1]) - (startSizes[index] + startSizes[index + 1])
