    ...
    >
      <div 
        ...
        className={cn(
          ...
          isDragOver ? "bg-[var(--color-accent)]/20 text-text-primary ring-1 ring-[var(--color-accent)]" : "hover:bg-white/5 hover:text-text-primary"
        )}
        data-dragover={isDragOver}
      >
        <div className="p-0.5 flex items-center justify-center">
          <FolderIcon size={14} className="group-hover:text-[var(--color-accent)] transition-colors" />
        </div>
        {editingFolderId === folder.id ? (
          ...
          <span className="text-[12px] font-semibold tracking-wide flex-1 truncate select-none">{folder.name}</span>
        )}
