...
export const MusicPlayer = () => {
  ...
  const artUrl = musicTrack?.artUrl || 'https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?auto=format&fit=crop&q=80&w=200&h=200'
  ...
  const handleSkipForward = (e: React.MouseEvent) => {
    ...
    const wv = document.getElementById(`webview-${musicTabId}`) as any
    if (!wv) return
    const code = tabUrl?.includes('youtube.com') ? `document.querySelector('.next-button')?.click()` : `document.querySelector('[data-testid="control-button-skip-forward"]')?.click()`
    wv.executeJavaScript(code)
  }
  ...
  const handleSkipBack = (e: React.MouseEvent) => {
    ...
    const wv = document.getElementById(`webview-${musicTabId}`) as any
    if (!wv) return
    const code = tabUrl?.includes('youtube.com') ? `document.querySelector('.previous-button')?.click()` : `document.querySelector('[data-testid="control-button-skip-back"]')?.click()`
    wv.executeJavaScript(code)
  }
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-5 mt-2">
            ...
            <div className="flex flex-col flex-1 overflow-hidden text-left">
              <span className="text-[14px] font-bold text-white truncate leading-tight">{title}</span>
          ...
          <div className="w-full flex items-center gap-2 mb-4">
            ...
            <div className="flex-1 h-1 bg-white/10 rounded-full cursor-pointer group relative">
              <div className="absolute top-0 left-0 h-full bg-white transition-all duration-300 rounded-full group-hover:bg-[var(--color-accent)]" style={{ width: `${progress}%` }} />
            </div>
          ...
          <div className="flex items-center justify-between px-2">
            <button className="text-white/40 hover:text-white transition-colors"><Heart size={16} /></button>
            <div className="flex items-center gap-4">
              <button onClick={handleSkipBack} className="text-white/80 hover:text-white transition-colors">
              ...
              <button onClick={handleSkipForward} className="text-white/80 hover:text-white transition-colors">
                <SkipForward size={20} fill="currentColor" />
              </button>
            </div>
            <button className="text-white/40 hover:text-white transition-colors"><Volume2 size={16} /></button>
          </div>
        </motion.div>
      </AnimatePresence>,
      document.body
    ...
    return (
      <div className="w-full flex justify-center py-2 relative group shrink-0" ref={miniPlayerRef}>
        <button 
          ...
          className={cn("w-9 h-9 rounded-xl flex items-center justify-center transition-colors no-drag music-trigger", isPlaying ? "bg-[var(--color-accent)]/10 text-[var(--color-accent)]" : "hover:bg-white/10 text-text-secondary")}
        >
          {isPlaying ? (
            <div className="flex items-end gap-[2px] h-3">
              <motion.div animate={{ height: [4, 12, 4] }} transition={{ repeat: Infinity, duration: 0.8 }} className="w-[3px] bg-current rounded-full" />
              <motion.div animate={{ height: [8, 4, 8] }} transition={{ repeat: Infinity, duration: 0.9 }} className="w-[3px] bg-current rounded-full" />
              <motion.div animate={{ height: [6, 10, 6] }} transition={{ repeat: Infinity, duration: 0.7 }} className="w-[3px] bg-current rounded-full" />
            </div>
          ) : (
            <Music2 size={16} />
          )}
    ...
    <div className="mx-3 mb-4 shrink-0">
      <motion.div 
        ...
        className={cn(
          ...
          isMusicPlayerOpen ? "bg-white/[0.05] border-white/[0.1] shadow-lg p-4" : "hover:bg-white/[0.04] p-2 h-[70px]"
        )}
        onClick={() => { if (!isMusicPlayerOpen) toggleMusicPlayer() }}
      >
        {isMusicPlayerOpen ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
            ...
            <div className="flex justify-between items-center mb-4 w-full">
              ...
              >
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M19 15l-7 7-7-7"/></svg>
              </button>
            </div>
          ...
          // COMPACT WIDGET (70px)
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-3 w-full h-full relative">
            <div className="w-[52px] h-[52px] rounded-xl overflow-hidden shrink-0 shadow-md relative">
              <img src={artUrl} alt="Album Art" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                {isPlaying ? (
                  <div className="flex items-end gap-[2px] h-3">
                    <motion.div animate={{ height: [4, 12, 4] }} transition={{ repeat: Infinity, duration: 0.8 }} className="w-[3px] bg-white rounded-full" />
                    <motion.div animate={{ height: [8, 4, 8] }} transition={{ repeat: Infinity, duration: 0.9 }} className="w-[3px] bg-white rounded-full" />
                    <motion.div animate={{ height: [6, 10, 6] }} transition={{ repeat: Infinity, duration: 0.7 }} className="w-[3px] bg-white rounded-full" />
                  </div>
                ) : null}
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 pr-1" onClick={(e) => e.stopPropagation()}>
              <button className="w-6 h-6 flex items-center justify-center text-white/40 hover:text-white transition-colors" onClick={handleSkipBack}>
              ...
              <button className="w-6 h-6 flex items-center justify-center text-white/40 hover:text-white transition-colors" onClick={handleSkipForward}>
